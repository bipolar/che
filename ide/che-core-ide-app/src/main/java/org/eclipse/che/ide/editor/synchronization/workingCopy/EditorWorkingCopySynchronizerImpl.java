...
public class EditorWorkingCopySynchronizerImpl implements EditorWorkingCopySynchronizer {
    ...
    public void synchronize(String filePath, String projectPath, DirtyRegion dirtyRegion) {
        ...
        JsonRpcPromise promise = requestTransmitter.newRequest()
                                                   .endpointId(ENDPOINT_ID)
                                                   .methodName(EDITOR_CONTENT_CHANGES_METHOD)
                                                   .paramsAsDto(changes)
                                                   .sendAndReceiveResultAsEmpty();
        promise.onFailure((Consumer<JsonRpcError>)jsonRpcError -> Log.error(getClass(), jsonRpcError.getMessage()));
    }

    private void onError(String endpointId, ServerError error) {
